...
const Navigation: React.FC<{ currentUser: User | null, handleLogout: () => void }> = ({ currentUser, handleLogout }) => {
  ...
  return (
    <>
      <nav className="fixed top-2 md:top-4 left-1/2 -translate-x-1/2 z-50 w-[96%] md:w-[95%] max-w-7xl px-3 md:px-10 py-3 md:py-4 flex justify-between items-center bg-stone-900/85 backdrop-blur-2xl border border-amber-900/30 rounded-[1.2rem] md:rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
        <div 
          className="flex items-center gap-2 md:gap-5 cursor-pointer group"
          onClick={() => window.location.hash = '#/profile'}
        >
          <div className="w-9 h-9 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-gradient-to-br from-amber-700/20 to-amber-900/40 flex items-center justify-center shadow-lg border border-amber-500/20 group-hover:scale-110 group-hover:border-amber-500/50 transition-all">
            <AvatarIcon size={20} className="text-amber-400 animate-float" />
          </div>
          <div className="flex flex-col">
            <span className="hidden lg:block font-medieval text-[8px] md:text-[10px] text-amber-500/60 uppercase tracking-[0.4em] mb-0.5">Caminhante do Reino</span>
            <span className="font-medieval text-xs md:text-xl text-stone-100 tracking-wider group-hover:text-amber-400 transition-colors whitespace-nowrap overflow-hidden text-ellipsis max-w-[80px] sm:max-w-none">
        ...
        {currentUser && (
          <div className="flex items-center gap-1.5 md:gap-8">
            <div 
              className="flex items-center gap-2 md:gap-3 text-stone-300 hover:text-amber-400 transition-all cursor-pointer group px-2.5 py-2 md:px-5 md:py-2.5 rounded-xl md:rounded-2xl hover:bg-amber-500/10 border border-transparent hover:border-amber-500/20" 
              onClick={() => window.location.hash = '#/dashboard'}
            >
              <LayoutGrid size={20} className="group-hover:scale-110 transition-transform" />
              <span className="hidden sm:inline text-xs md:text-lg font-medieval uppercase tracking-[0.2em]">Grimório</span>
            </div>
            
            <div 
              className="hidden md:flex flex-col items-end border-r border-amber-900/30 pr-4 md:pr-8 mr-1 md:mr-2 cursor-pointer group"
              onClick={() => window.location.hash = '#/profile'}
            >
              ...
              <div className="flex items-center gap-2 text-stone-500 font-story tracking-wider uppercase group-hover:text-amber-500/60 transition-colors">
                <span className="text-[10px]">Configurar</span>
                <Settings size={8} />
              </div>
            </div>
