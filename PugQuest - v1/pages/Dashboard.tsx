...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-24 px-6 md:px-12 pt-8 max-w-7xl mx-auto">
      <header className="mb-16 text-center md:text-left border-b border-amber-900/10 pb-12">
        <h1 className="text-4xl md:text-6xl font-medieval text-stone-100 mb-4 uppercase tracking-widest amber-glow">Salão de Heróis</h1>
        <p className="text-stone-400 font-story text-xl max-w-2xl">Escolha seu caminho no grande livro das eras.</p>
      ...
      <div className="space-y-24">
        <section>
          <div className="flex items-center gap-4 mb-10 group">
            ...
            <h2 className="text-xl font-medieval text-stone-200 uppercase tracking-widest">Grimórios que Você Mestra</h2>
          </div>
        ...
        <section>
          <div className="flex items-center gap-4 mb-10 group">
            ...
            <h2 className="text-xl font-medieval text-stone-200 uppercase tracking-widest">Caminhos Trilhandos</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="Forjar Nova Crônica">
        <form onSubmit={handleCreateCampaign} className="space-y-8">
          <div>
            <label className="text-xs font-medieval uppercase tracking-widest text-amber-500 mb-3 block">Título da Saga</label>
            <input required value={newCampaign.name} onChange={e => setNewCampaign({...newCampaign, name: e.target.value})} className="w-full bg-stone-950 border border-amber-900/30 rounded-2xl p-5 text-stone-100 focus:border-amber-500 outline-none font-story text-xl shadow-inner" placeholder="Ex: O Retorno do Rei-Bruxo" />
          </div>
          <div>
            <label className="text-xs font-medieval uppercase tracking-widest text-amber-500 mb-3 block">Prefácio do Mundo</label>
            <textarea rows={4} value={newCampaign.description} onChange={e => setNewCampaign({...newCampaign, description: e.target.value})} className="w-full bg-stone-950 border border-amber-900/30 rounded-2xl p-5 text-stone-100 focus:border-amber-500 outline-none resize-none font-story text-lg shadow-inner" placeholder="Narração curta para os viajantes..." />
          </div>
      ...
      <Modal isOpen={isJoinModalOpen} onClose={() => setIsJoinModalOpen(false)} title="Entrar com Runa" size="md">
        <form onSubmit={handleJoinCampaign} className="space-y-10">
          <div className="text-center">
            <div className="w-20 h-20 rounded-full bg-amber-500/10 flex items-center justify-center mx-auto mb-6 border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.1)]">
              <Key className="text-amber-500 animate-float" size={32} />
            </div>
            <p className="text-stone-400 font-story text-lg mb-8">Insira o código sagrado de 6 caracteres fornecido pelo seu mestre para se vincular a este reino.</p>
          </div>
          ...
          <div>
            <input 
              ...
              className="w-full bg-stone-950 border-2 border-amber-900/30 rounded-2xl p-6 text-center text-5xl font-mono tracking-[0.4em] text-amber-500 focus:border-amber-500 outline-none transition-all uppercase shadow-inner"
              placeholder="XXXXXX"
              maxLength={6}
            />
          </div>
          ...
          {joinError && (
            <motion.div 
              ...
              className="p-4 bg-red-950/20 border border-red-900/30 rounded-xl text-center"
            >
              <p className="text-red-500 text-xs font-medieval uppercase tracking-widest">{joinError}</p>
            </motion.div>
          )}

          <Button fullWidth size="lg" type="submit" disabled={isJoining} variant="primary" className="text-lg">
    ...
    <Card onClick={() => { window.location.hash = `#/campaign/${campaign.id}`; }} className="!p-0 border-amber-900/10 group h-full">
      <div className="relative aspect-[16/10] overflow-hidden rounded-t-[1.4rem]">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          <div className={`px-4 py-1.5 rounded-xl border backdrop-blur-md text-[9px] uppercase font-medieval tracking-widest font-bold ${role === 'MASTER' ? 'bg-amber-950/60 text-amber-400 border-amber-500/30' : 'bg-blue-950/60 text-blue-400 border-blue-500/30'}`}>
            {role === 'MASTER' ? 'Mestre' : 'Viajante'}
          </div>
          <div className={`px-4 py-1.5 rounded-xl border backdrop-blur-md text-[9px] uppercase font-medieval tracking-widest font-bold ${getStatusColor(campaign.status)}`}>
            {campaign.status}
          </div>
        </div>
      </div>
      <div className="p-8">
        <h3 className="text-2xl font-medieval text-stone-100 mb-4 uppercase tracking-wide group-hover:text-amber-400 transition-colors leading-tight">
          {campaign.name}
        </h3>
        <p className="text-stone-400 font-story text-[15px] leading-relaxed line-clamp-2 mb-6">
          {campaign.description || "O tomo aguarda sua primeira escrita..."}
        </p>

        <div className="flex items-center justify-between border-t border-white/5 pt-6">
          <div className="flex items-center gap-3 text-stone-500 text-[11px] font-medieval uppercase tracking-widest">
            <Users size={14} /> {membersCount} {membersCount === 1 ? 'Jogador' : 'Jogadores'}
          </div>
          
          <button 
            onClick={copyCode}
            className="flex items-center gap-2 text-stone-500 hover:text-amber-500 transition-all text-[11px] font-medieval uppercase tracking-widest group/btn"
          >
...
const EmptyState: React.FC<{ message: string }> = ({ message }) => (
  <div className="col-span-full py-20 flex flex-col items-center justify-center bg-stone-900/40 rounded-3xl border border-dashed border-amber-900/20">
    <ScrollText className="text-stone-700 mb-4" size={48} />
    <p className="text-stone-500 font-medieval tracking-[0.2em] uppercase text-xs">{message}</p>
  </div>
);
