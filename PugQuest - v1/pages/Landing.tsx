...
const Landing: React.FC = () => {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden relative">
      <header className="absolute top-0 left-0 w-full pt-12 pb-6 z-20 flex flex-col items-center">
        <motion.div
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-medieval text-4xl md:text-5xl text-stone-100 tracking-[0.4em] uppercase amber-glow">
              Pug Quest
            </h1>
            <Sword className="text-amber-500/40 w-5 h-5 scale-x-[-1]" />
            <div className="h-px w-12 bg-gradient-to-l from-transparent to-amber-500/50" />
          </div>
          <span className="font-medieval text-[9px] text-amber-500/40 tracking-[0.8em] uppercase ml-4">O Guia do Mestre</span>
        </motion.div>
      </header>
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <motion.div
              initial={{ opacity: 0, y: -20 }}
              animate={{ opacity: 1, y: 0 }}
              className="inline-flex items-center gap-3 bg-amber-950/40 border border-amber-700/30 rounded-2xl px-5 py-2 backdrop-blur-md mb-8"
            >
              <Sparkles className="w-4 h-4 text-amber-500 animate-pulse" />
              <span className="font-medieval text-amber-200 text-[10px] font-bold tracking-[0.3em] uppercase">Crônicas de uma Nova Era</span>
            </motion.div>

            <h1 className="font-medieval text-6xl md:text-8xl text-stone-100 leading-[1.1] mb-8 amber-glow uppercase">
              Esculpa Seu <br/>
            ...
            <p className="font-story text-xl md:text-2xl text-stone-300/80 mb-12 max-w-xl mx-auto lg:mx-0 leading-relaxed border-l-2 border-amber-900/50 pl-6 uppercase">
            ...
            <div className="flex flex-wrap gap-6 justify-center lg:justify-start">
              <Button onClick={() => { setAuthMode('register'); setIsAuthModalOpen(true); }} size="lg" className="shadow-amber-900/20">
      ...
      <section id="features" className="py-32 px-4 relative">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-24">
            <h2 className="font-medieval text-4xl text-stone-100 mb-6 uppercase tracking-[0.2em] amber-glow">Tradições do Reino</h2>
            <div className="w-40 h-1 bg-gradient-to-r from-transparent via-amber-600 to-transparent mx-auto rounded-full" />
          </div>
      ...
      <footer className="py-20 border-t border-amber-900/20 text-center relative z-10 bg-stone-950">
      ...
      >
        <AnimatePresence mode="wait">
          {isVerificationSent ? (
            ...
            >
              <div className="w-24 h-24 rounded-full bg-emerald-500/10 flex items-center justify-center mx-auto mb-8 border border-emerald-500/30 shadow-[0_0_30px_rgba(16,185,129,0.1)]">
                <Send className="text-emerald-500 animate-float" size={40} />
              </div>
              <h4 className="font-medieval text-2xl text-stone-100 mb-4 uppercase tracking-widest amber-glow">O Vínculo Está Quase Completo!</h4>
              <p className="text-stone-300 font-story text-lg mb-8 leading-relaxed">
            ...
            <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="text-center mb-10">
                <div className="w-16 h-16 rounded-full bg-amber-500/10 flex items-center justify-center mx-auto mb-6 border border-amber-500/30 shadow-[0_0_20px_rgba(251,191,36,0.1)]">
                  <ShieldCheck className="text-amber-500" size={28} />
                </div>
                <p className="text-amber-200/50 font-story text-base uppercase">
              ...
              <form onSubmit={handleAuthSubmit} className="space-y-6">
                {authMode === 'register' && (
                  <div>
                    <label className="text-[10px] font-medieval uppercase tracking-widest text-amber-500 mb-2 block">Nome do Herói / Mestre</label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-stone-950 border border-amber-900/40 rounded-xl p-3.5 pl-12 text-stone-100 focus:border-amber-500 outline-none transition-all font-story text-lg shadow-inner"
                ...
                <div>
                  <label className="text-[10px] font-medieval uppercase tracking-widest text-amber-500 mb-2 block">Vínculo Espiritual (Email)</label>
                  <div className="relative">
                ...
                <div>
                  <label className="text-[10px] font-medieval uppercase tracking-widest text-amber-500 mb-2 block">Palavra-Passe</label>
                  <div className="relative">
                ...
                {authMode === 'register' && (
                  <div>
                    <label className="text-[10px] font-medieval uppercase tracking-widest text-amber-500 mb-2 block">Confirmar Pacto</label>
                    <div className="relative">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-amber-500/40" size={16} />
                      <input 
                        required
                ...
                <Button fullWidth size="lg" type="submit" disabled={isLoading} className="shadow-lg">
                  {isLoading ? 'Conjurando...' : authMode === 'login' ? 'Entrar no Reino' : 'Forjar Minha Conta'}
                </Button>
                <div className="text-center mt-6">
                  <button type="button" onClick={toggleAuthMode} className="text-amber-500/60 hover:text-amber-400 text-[11px] font-medieval uppercase tracking-[0.2em] transition-colors">
...
const FeatureCard: React.FC<{ icon: React.ReactNode, title: string, description: string }> = ({ icon, title, description }) => (
  <Card className="!p-10 flex flex-col items-center text-center group border-amber-900/10">
    <div className="w-16 h-16 rounded-2xl bg-amber-950/40 flex items-center justify-center mb-8 border border-amber-500/20 group-hover:bg-amber-600 group-hover:text-amber-100 transition-all text-amber-500">
      {icon}
    </div>
    <h3 className="font-medieval text-xl text-stone-100 mb-4 uppercase tracking-widest group-hover:text-amber-400 transition-colors">{title}</h3>
    <p className="font-story text-stone-400 leading-relaxed uppercase">{description}</p>
  </Card>
);
